...
public class LWJGLPointLight {
	...
	public static void init(int width, int height) {
		...
		try {
			fragment.setSource(new File(LWJGLTest.class.getResource("/shaders/lighting.frag").toURI()));
			fragment.compile();
			vertex.setSource(new File(LWJGLTest.class.getResource("/shaders/lighting.vert").toURI()));
			vertex.compile();
	...
	public void setPosition(Vector2f position) { m_position = position; }
