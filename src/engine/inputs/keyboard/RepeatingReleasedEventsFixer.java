 ...
 * bug in the Linux KeyEvent handling for keyboard repeat. Linux apparently
 * implements repeating keypresses by
 * repeating both the {@link KeyEvent#KEY_PRESSED} and
 * {@link KeyEvent#KEY_RELEASED}, while on Windows, one only
 * gets repeating PRESSES, and then a final RELEASE when the key is released.
 * The Windows way is obviously much more
 * useful, as one then can easily distinguish between a user holding a key
 * pressed, and a user hammering away on the
 * key.
 * 
 ...
 * but it is simpler to invoke {@link #install() install(new
 * instance)}:
 * 
 * 
 * 
 ...
 * your application have some "reboot"
 * functionality that can potentially install it again - or else you'll end up
 * with multiple instances, which isn't too
 * hot.
 * 
 * Notice: Read up on the {@link Reposted} interface if you have other
 * AWTEventListeners that resends KeyEvents
 ...
 * after the RELEASED event - one thus
 ...
 * soon afterwards, the RELEASED is dropped
 * (consumed) - while if the timer times out, the event is reposted and thus
 * becomes the final, wanted RELEASED that
...
public class RepeatingReleasedEventsFixer implements AWTEventListener {
	...
	public void eventDispatched(AWTEvent event) {
		...
		if (keyEvent.getID() == KeyEvent.KEY_RELEASED) {
			 ...
			 * subsequent PRESSED shall already have been
			 * posted on the event queue, and shall thus be the direct next
			 * event no matter which events are posted
			 * afterwards. The code with the ReleasedAction handles if the Timer
			 * thread actually fires the action due to
	 ...
	 * the {@link Timer} times out (and hence the
	 ...
	 * that the event shall not be "hack processed" by this class
	 * again. (The problem is that it is not possible to state
	 * "inject this event from this point in the pipeline" - one
	 * have to inject it to the event queue directly, thus it will come through
