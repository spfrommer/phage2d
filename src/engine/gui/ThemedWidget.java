...
public abstract class ThemedWidget extends Widget {
	...
	public void renderWidget(Renderer renderer) {
		...
		if (image != null) {
			...
			image.render(renderer, getAnimationState(), 0, 0, getSize()
	...
	public Dimension getMinimumSize() {
		...
		if (theme != null) {
			...
			/*
			 * if (item != null && item instanceof Parameter) { Image image =
			 * (Image) ((Parameter) item).getValue(); logger.info("Width: " +
			 * minWidth + " Height: " + minHeight); return new
			 * Dimension(image.getMinWidth(), image.getMinHeight()); }
			 */
			ThemeItem mh = theme.get("minHeight");
			ThemeItem mw = theme.get("minWidth");
			if (mh != null && mh instanceof Parameter) {
				minHeight = (Integer) ((Parameter) mh).getValue();
			}
			if (mw != null && mw instanceof Parameter) {
				minWidth = (Integer) ((Parameter) mw).getValue();
			}
		}
