...
public class ShaceshipServer extends Server {
	...
	public void onStart() {
		...
		this.getEntitySystem().addEntity(
				makeWall(new Vector(5050, 0), 100, 10000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-5050, 0), 100, 10000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(0, 5050), 10000, 100));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(0, -5050), 10000, 100));

		this.getEntitySystem().addEntity(
				makeWall(new Vector(1000, 550), 1000, 100));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(300, 2050), 100, 800));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(0, -1900), 100, 1000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-1000, 500), 1000, 1000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(1500, -1500), 900, 500));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(3000, 3000), 2000, 500));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(3000, -2000), 1000, 1000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-2000, 3000), 2000, 2000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-3300, 1000), 2000, 200));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-2500, 0), 500, 500));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(4000, 0), 1000, 200));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-3700, -3700), 500, 1000));
		this.getEntitySystem().addEntity(
				makeWall(new Vector(-3700, -2000), 1500, 1000));
	}
	...
	private Entity makeBackground() {
		...
		ComponentFactory.addTextureData(background,
				new Texture(ImageUtils.getID("terrain1.jpg"), 10000, 10000));
		...
		background
				.addComponent(ComponentFactory.createBasicEncoder(background));
	...
	private Entity makeWall(Vector position, double width, double height) {
		...
		PhysicsData physics = ComponentFactory.addPhysicsData(wall, position,
				0, new Rectangle(width, height));
		physics.setMassType(Mass.Type.INFINITE);

		ComponentFactory.addTextureData(wall,
				new Texture(ImageUtils.getID("darkmetal.jpg"), width, height));
	...
	private static String[] chassisImages = new String[] { "gun1.png",
			"chassis2.png", "gun2.png", "bullet1.png" };

	private Entity makeBumperCar(Vector position, int clientID) {
		Entity bumperCar = new Entity();

		PhysicsData physics = ComponentFactory.addPhysicsData(bumperCar,
				position, 0, new Rectangle(140, 250));
		...
		ComponentFactory.addTextureData(bumperCar, new Texture(imageID, 140,
				250));
		...
		encoder.addDataEncoder(TypeManager.getType(CameraFocusData.class),
				new BasicDataEncoder());
		...
		bumperCar.addComponent(new GunLogic(bumperCar,
				makeBullet(bumperCar, 5), input, 10, 3000));
	...
	private Entity makeBullet(Entity exclude, double bulletDamage) {
		...
		PhysicsData physics = ComponentFactory.addPhysicsData(bullet,
				new Vector(0, 0), 0, new Rectangle(10, 30));
		physics.setMass(1000);

		ComponentFactory.addTextureData(bullet,
				new Texture(ImageUtils.getID("bullet1.png"), 10, 30));
		...
		bullet.addComponent(new BulletCollisionHandlerLogic(bullet, this
				.getEntitySystem()));
	...
	public InputManager makeInputManager(NetworkInputHub inputHub, int clientID) {
		...
		addInputTrigger(networkInputManager, inputHub, "inputforwards",
				"Forwards", clientID);
		addInputTrigger(networkInputManager, inputHub, "inputbackwards",
				"Backwards", clientID);
		addInputTrigger(networkInputManager, inputHub, "inputright", "Right",
				clientID);
		addInputTrigger(networkInputManager, inputHub, "inputleft", "Left",
				clientID);
		addInputTrigger(networkInputManager, inputHub, "inputT", "t", clientID);

		addInputTrigger(networkInputManager, inputHub, "inputmousex",
				"MouseWorldX", clientID);
		addInputTrigger(networkInputManager, inputHub, "inputmousey",
				"MouseWorldY", clientID);
		addInputTrigger(networkInputManager, inputHub, "inputleftmousedown",
				"LeftMouse", clientID);
		addInputTrigger(networkInputManager, inputHub, "inputrightmousedown",
				"RightMouse", clientID);

		return networkInputManager;
	}

	private void addInputTrigger(InputManager inputManager,
			NetworkInputHub inputHub, String message, String binding, int id) {
	...
	public static void main(String[] args) {
		...
		ShaceshipServer server = new ShaceshipServer(new CommandInterpreter(
				declarations), Server.PORT);
		server.start();
	}
}
